...
public class PickStatusChangeTests
{
    ...
    {
        var sut = new SlotStatusChanger(statusTable);
    ...
    [Test]
    public void Func_전달시_정상적으로_적용된다()
    {
        var sut = new SlotStatusChanger(statusTable);

        var slot = new SlotData(Team.Blue, 0);
        var before = statusTable.GetSlot(slot).Stat;

        sut.ChangeStat(slot, stat => new ChampionStatData(15, stat.Defense, stat.Speed));

        var result = statusTable.GetSlot(slot).Stat;
        Assert.AreEqual(15, result.Attack);
        Assert.AreEqual(before.Defense, result.Defense);
        Assert.AreEqual(before.Speed, result.Speed);
    }
}
