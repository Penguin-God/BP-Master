...
public class MatchRunnerTests
{
    ...
    public void AI_배틀을_실행시키고_결과를_반환해야_함()
    {
        ...
        var blueBanPick = new AI_BanPickAgent(Team.Blue, storage, blueSelector, blueSelector);

        var redSelector = new PrioritySelector(new int[] { 4 }, new int[] { 5, 6 });
        var redBanPick = new AI_BanPickAgent(Team.Red, storage, redSelector, redSelector);
    }
